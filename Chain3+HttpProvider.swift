...
public protocol Chain3Provider {
...
public class Chain3HttpProvider: Chain3Provider {
    /// node url address
    public var url: URL
    /// network id which used for local signing
    ...
    public init?(_ httpProviderURL: URL, network net: NetworkId? = nil, keystoreManager manager: KeystoreManager? = nil) {
        do {
            ...
            if net == nil {
                ...
                let response = try Chain3HttpProvider.post(request, providerURL: httpProviderURL, queue: DispatchQueue.global(qos: .userInteractive), session: session).wait()
        ...
        queue.async {
            do {
                ...
                task = session.dataTask(with: urlRequest) { data, _, error in
                    ...
                    guard data != nil else {
                        rp.resolver.reject(Chain3Error.nodeError("Node response is empty"))
                        return
                    }
        ...
        return rp.promise.ensure(on: queue) {
            task = nil
                ...
                if parsedResponse.error != nil {
                    throw Chain3Error.nodeError("Received an error message from node\n" + String(describing: parsedResponse.error!))
                }
    ...
    public func sendAsync(_ request: JsonRpcRequest, queue: DispatchQueue = .main) -> Promise<JsonRpcResponse> {
        return Chain3HttpProvider.post(request, providerURL: url, queue: queue, session: session)
    }
    
    public func sendAsync(_ requests: JsonRpcRequestBatch, queue: DispatchQueue = .main) -> Promise<JsonRpcResponseBatch> {
        return Chain3HttpProvider.post(requests, providerURL: url, queue: queue, session: session)
    }
}
