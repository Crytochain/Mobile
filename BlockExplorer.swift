...
public class BlockExplorer {
    ...
    public func getTransactionHistory(address: Address, tokenName name: String = "LBR", page: Int = 1, size: Int = 50) -> Promise<[TransactionHistoryRecord]> {
        let address = address.address
        return getTransactionsHistory(address: address, tokenName: name, page: page, size: size)
    }
    
    public func getTransactionsHistory(address publicAddress: String, tokenName name: String = "LBR", page: Int = 1, size: Int = 50) -> Promise<[TransactionHistoryRecord]> {
        // Configuring http request
        let listId: ListId = (name == "LBR") ? .listOfMC : .listOfTokens
