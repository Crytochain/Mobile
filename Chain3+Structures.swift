...
extension Chain3Options: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if toString == nil || toString == "0x" || toString == "0x0" {
            ...
            let LBRAddr = Address(addressString)
            guard LBRAddr.isValid else { throw Chain3Error.dataError }
            to = LBRAddr
        }
        self.to = to
...
//        var from: Address?
//        if fromString != nil {
//            guard let LBRAddr = Address(toString) else { throw Chain3Error.dataError }
//            from = LBRAddr
//        }
        self.from = from
...
extension LBRTransaction: Decodable {
    enum CodingKeys: String, CodingKey {
        case to
        case data
        case input
...
public struct TransactionDetails: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        let transaction = try LBRTransaction(from: decoder)
    ...
    public init(_ json: [String: Any]) throws {
        ...
        transaction = try LBRTransaction(json)
...
public struct TransactionReceipt: Decodable {
    ...
    public init(transactionHash: Data, blockHash: Data, blockNumber: BigUInt, transactionIndex: BigUInt, contractAddress: Address?, cumulativeGasUsed: BigUInt, gasUsed: BigUInt, logs: [EventLog], status: TXStatus, logsBloom: LBRBloomFilter?) {
        self.transactionHash = transactionHash
        self.blockHash = blockHash
        self.blockNumber = blockNumber
        self.transactionIndex = transactionIndex
...
public enum TransactionInBlock: Decodable {
    case hash(Data)
    case transaction(LBRTransaction)
    case null
...
//            guard let t = try? LBRTransaction(from: decoder) else { throw Chain3Error.dataError }
            let t = try LBRTransaction(dict)
            self = .transaction(t)
        } else {
            self = .null
        }
...
public struct Block: Decodable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        var bloom: LBRBloomFilter?
        if logsBloomData != nil {
            bloom = LBRBloomFilter(logsBloomData!)
        }
        logsBloom = bloom
